...
robot_b64 = ""
try:
    with open("assets/robot.png", "rb") as f:
        robot_b64 = base64.b64encode(f.read()).decode()
    print(f"Robot image loaded: {len(robot_b64)} chars")
...
img_tag = (
    ...
    'filter:drop-shadow(0 0 35px rgba(0,191,255,0.65)) drop-shadow(0 0 70px rgba(251,183,36,0.3)) brightness(1.1);'
    'animation:float 3s ease-in-out infinite;position:relative;z-index:2;display:block;margin:0 auto;"/>'
    if robot_b64 else
    '<div style="font-size:9rem;text-align:center;">🤖</div>'
)
...
# Build new home page — use regular string (no f-string) to avoid conflicts
new_home = '''if page == "🏠 Home":
    ...
    with hero_left:
        st.markdown("""
        <div class="hero-section" style="min-height:340px;">
            <div class="hero-badge">&#10022; Powered by Azure OpenAI + RAG</div>
            <div class="hero-title">AI-Powered<br>Text-to-SQL<br>Assistant</div>
            <div class="hero-sub">
                Transform plain English into precise SQL queries instantly.
                No SQL expertise needed — just ask your business question and get results.
            </div>
            <div style="display:flex;gap:0.7rem;flex-wrap:wrap;margin-top:1rem;">
                <span style="background:rgba(251,183,36,0.1);border:1px solid rgba(251,183,36,0.3);
                    border-radius:8px;padding:0.35rem 0.8rem;font-size:0.75rem;color:#FBB724;font-weight:600;">
                    &#9889; GPT-4 Powered</span>
                <span style="background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.3);
                    border-radius:8px;padding:0.35rem 0.8rem;font-size:0.75rem;color:#10B981;font-weight:600;">
                    &#128994; Live Database</span>
                <span style="background:rgba(139,92,246,0.1);border:1px solid rgba(139,92,246,0.3);
                    border-radius:8px;padding:0.35rem 0.8rem;font-size:0.75rem;color:#8B5CF6;font-weight:600;">
...
''' + '''        st.markdown(f"""
        ...
        <div style="
            background: radial-gradient(ellipse at center,
                rgba(0,191,255,0.08) 0%, rgba(251,183,36,0.05) 50%, transparent 80%);
        ...
        ">
            <div style="position:absolute;top:50%;left:50%;width:290px;height:290px;
                border-radius:50%;border:1px solid rgba(0,191,255,0.07);
                transform:translate(-50%,-50%);pointer-events:none;"></div>
            <div style="position:absolute;top:50%;left:50%;width:220px;height:220px;
            ...
            <div style="position:absolute;top:12%;left:3%;
                ...
                border-radius:8px;padding:0.25rem 0.65rem;font-size:0.65rem;
                color:#00BFFF;font-weight:700;font-family:monospace;
                box-shadow:0 0 10px rgba(0,191,255,0.2);">SELECT *</div>
            <div style="position:absolute;top:16%;right:3%;
                background:rgba(251,183,36,0.12);border:1px solid rgba(251,183,36,0.4);
                border-radius:8px;padding:0.25rem 0.65rem;font-size:0.65rem;
                color:#FBB724;font-weight:700;font-family:monospace;
                box-shadow:0 0 10px rgba(251,183,36,0.2);">JOIN &#10003;</div>
            <div style="position:absolute;bottom:28%;left:2%;
                background:rgba(16,185,129,0.12);border:1px solid rgba(16,185,129,0.4);
                border-radius:8px;padding:0.25rem 0.65rem;font-size:0.65rem;
                color:#10B981;font-weight:700;font-family:monospace;
                box-shadow:0 0 10px rgba(16,185,129,0.2);">10 rows</div>
            <div style="position:absolute;bottom:26%;right:2%;
                background:rgba(139,92,246,0.12);border:1px solid rgba(139,92,246,0.4);
                border-radius:8px;padding:0.25rem 0.65rem;font-size:0.65rem;
                color:#8B5CF6;font-weight:700;font-family:monospace;
                box-shadow:0 0 10px rgba(139,92,246,0.2);">GPT-4</div>

            <div style="margin-top:1rem;display:flex;align-items:center;
                justify-content:center;gap:7px;z-index:2;position:relative;">
                ...
                <span style="font-size:0.75rem;color:#10B981;font-weight:700;
                    letter-spacing:0.06em;">AI AGENT ONLINE</span>
            </div>
    ...
    if st.session_state.submit and st.session_state.get("question_input","").strip():
        ...
        if _uq.lower() in {"hi","hello","hey"}:
            ...
            if _err:
                ...
                with _rc:
                    st.markdown('<div style="font-size:0.78rem;font-weight:700;color:#FBB724;'
                                'text-transform:uppercase;letter-spacing:0.08em;margin-bottom:0.4rem;">'
                                '&#128202; Results</div>', unsafe_allow_html=True)
                    if _df is not None and not _df.empty:
                        st.markdown(f"<span style='color:#10B981;font-size:0.8rem;font-weight:600;'>"
                                    f"&#10003; {len(_df)} row(s) &middot; {len(_df.columns)} col(s)</span>",
                                    unsafe_allow_html=True)
                ...
                with _sc:
                    st.markdown('<div style="font-size:0.78rem;font-weight:700;color:#FBB724;'
                                'text-transform:uppercase;letter-spacing:0.08em;margin-bottom:0.4rem;">'
                                '&#128295; Generated SQL</div>', unsafe_allow_html=True)
                    ...
                    if _df is not None and not _df.empty:
                        ...
                        if _num and _cat:
                            ...
                            st.markdown('<div style="font-size:0.78rem;font-weight:700;color:#FBB724;'
                                        ...
                                        '&#128200; Auto Chart</div>', unsafe_allow_html=True)
                            _fig = px.bar(_df.head(10), x=_cat[0], y=_num[0],
                                          color_discrete_sequence=["#FBB724"])
                            _fig.update_layout(
    ...
    <div style="display:flex;align-items:center;gap:10px;margin:2rem 0 1rem;">
        ...
        <span style="background:rgba(251,183,36,0.1);color:#FBB724;font-size:0.7rem;
            font-weight:700;padding:0.15rem 0.6rem;border-radius:20px;text-transform:uppercase;">
            Features</span>
    </div>
    """, unsafe_allow_html=True)
    _features = [
...
print("✓ Home page patched successfully")
