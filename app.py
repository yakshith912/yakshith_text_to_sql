...
[data-testid="stAppViewContainer"] .element-container p{color:#0F172A!important;}
...
/* ── Buttons ── */
.stButton>button{
    background:linear-gradient(135deg,#10B981,#FBB724)!important;
...
.feature-title{font-size:1rem;font-weight:700;color:#0F172A!important;margin-bottom:0.4rem;}
...
.toast-success{
    ...
    border-radius:10px;padding:0.7rem 1rem;color:#FBB724!important;
...
.section-label{
    font-size:0.68rem;font-weight:700;color:#10B981!important;
...
.upload-zone{
    background:#FFFFFF;border:2px dashed rgba(16,185,129,0.4);
...
with st.sidebar:
    # Logo
    st.markdown("""
    <div style="padding:1.5rem 1rem 1rem;border-bottom:1px solid rgba(0,0,0,0.06);margin-bottom:0.5rem;">
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <div>
                <div style="font-size:1rem;font-weight:800;color:#0F172A!important;">AaiTech</div>
...
if _page == "🏠 Home":
    ...
    <div style="background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:0.8rem;">
            ...
            <span style="font-size:0.9rem;font-weight:700;color:#0F172A!important;">&#9889; Ask the AI</span>
                ...
                with _sc:
                    ...
                    if _df is not None and not _df.empty:
                        ...
                        if _num and _cat:
                            ...
                            _fig.update_layout(paper_bgcolor="rgba(255,255,255,1)",plot_bgcolor="rgba(255,255,255,1)",
                                font=dict(color="#334155",size=10),height=220,margin=dict(t=5,b=5,l=5,r=5),
                                xaxis=dict(showgrid=False,color="#334155"),
                                yaxis=dict(showgrid=True,gridcolor="rgba(0,0,0,0.05)",color="#334155"))
    ...
    <div style="display:flex;align-items:center;gap:10px;margin:2rem 0 1rem;">
        <span style="font-size:1.1rem;font-weight:700;color:#0F172A!important;">Everything you need</span>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div style="margin-bottom:1.5rem;">
        ...
        <div style="font-size:0.9rem;color:#64748B!important;-webkit-text-fill-color:#64748B!important;margin-top:4px;">Ask any business question — get SQL + results instantly</div>
    </div>""", unsafe_allow_html=True)
    ...
    <div class="glass-card" style="margin-bottom:1rem;">
        ...
        <span style="display:inline-block;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.08);color:#64748B!important;font-size:0.78rem;padding:0.3rem 0.8rem;border-radius:20px;margin:0.2rem;">Show all customers</span>
        <span style="display:inline-block;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.08);color:#64748B!important;font-size:0.78rem;padding:0.3rem 0.8rem;border-radius:20px;margin:0.2rem;">Revenue by category</span>
        <span style="display:inline-block;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.08);color:#64748B!important;font-size:0.78rem;padding:0.3rem 0.8rem;border-radius:20px;margin:0.2rem;">Top 5 expensive products</span>
        <span style="display:inline-block;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.08);color:#64748B!important;font-size:0.78rem;padding:0.3rem 0.8rem;border-radius:20px;margin:0.2rem;">Orders by country</span>
        <span style="display:inline-block;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.08);color:#64748B!important;font-size:0.78rem;padding:0.3rem 0.8rem;border-radius:20px;margin:0.2rem;">Total freight per country</span>
        <span style="display:inline-block;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.08);color:#64748B!important;font-size:0.78rem;padding:0.3rem 0.8rem;border-radius:20px;margin:0.2rem;">Supplier with most products</span>
    </div>""", unsafe_allow_html=True)
        ...
        if _err:
            ...
            if _df is not None and not _df.empty:
                ...
                if _num and _cat:
                    ...
                    _fig.update_layout(paper_bgcolor="rgba(255,255,255,1)",plot_bgcolor="rgba(255,255,255,1)",
                        font=dict(color="#334155",size=11),height=320,margin=dict(t=10,b=10,l=10,r=10),
                        xaxis=dict(showgrid=False,color="#334155"),
                        yaxis=dict(showgrid=True,gridcolor="rgba(0,0,0,0.05)",color="#334155"))
                    st.plotly_chart(_fig, use_container_width=True)
            else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    BG="rgba(0,0,0,0)"; GRID="rgba(0,0,0,0.05)"
    FONT=dict(family="Inter,sans-serif",size=11,color="#334155")
    LO=dict(paper_bgcolor="rgba(255,255,255,1)",plot_bgcolor="rgba(255,255,255,1)",font=FONT,
    ...
    with _sub_tabs[0]:
        try:
            ...
            with _r1:
                ...
                st.markdown('<span style="font-size:0.82rem;font-weight:700;color:#0F172A;">Revenue by Category</span>', unsafe_allow_html=True)
            ...
            with _r2:
                st.markdown('<div class="glass-card" style="padding:1rem;">', unsafe_allow_html=True)
                st.markdown('<span style="font-size:0.82rem;font-weight:700;color:#0F172A;">Orders by Country</span>', unsafe_allow_html=True)
                ...
                if not _co.empty:
                    ...
                    _f2.update_layout(**LO,height=280,
                        xaxis=dict(showgrid=False,color="#334155"),
                        yaxis=dict(showgrid=True,gridcolor=GRID,color="#334155"))
            ...
            with _r3:
                st.markdown('<div class="glass-card" style="padding:1rem;">', unsafe_allow_html=True)
                st.markdown(f'<span style="font-size:0.82rem;font-weight:700;color:#0F172A;">Top {_tn} Products by Revenue</span>', unsafe_allow_html=True)
                ...
                if not _pr.empty:
                    ...
                    _f3.update_layout(**LO,height=280,
                        xaxis=dict(showgrid=True,gridcolor=GRID,color="#334155"),
            ...
            with _r4:
                st.markdown('<div class="glass-card" style="padding:1rem;">', unsafe_allow_html=True)
                st.markdown('<span style="font-size:0.82rem;font-weight:700;color:#0F172A;">Freight Cost Analysis</span>', unsafe_allow_html=True)
            ...
            with _r5:
                st.markdown('<div class="glass-card" style="padding:1rem;">', unsafe_allow_html=True)
                st.markdown('<span style="font-size:0.82rem;font-weight:700;color:#0F172A;">Product Revenue Treemap</span>', unsafe_allow_html=True)
                ...
                if not _tree.empty:
                    ...
                    _f5.update_layout(margin=dict(t=10,b=10,l=10,r=10),height=340,
                        coloraxis_showscale=False,paper_bgcolor="rgba(255,255,255,1)",
            ...
            with _r6:
                st.markdown('<div class="glass-card" style="padding:1rem;">', unsafe_allow_html=True)
                st.markdown('<span style="font-size:0.82rem;font-weight:700;color:#0F172A;">Supplier Performance</span>', unsafe_allow_html=True)
                ...
                _f6.update_layout(margin=dict(t=5,b=5,l=5,r=30),height=340,
                    paper_bgcolor="rgba(255,255,255,1)",plot_bgcolor="rgba(255,255,255,1)",font=FONT,
                    xaxis=dict(showgrid=True,gridcolor=GRID,color="#334155"),
            ...
            <div style="text-align:center;padding:1rem 0 0.5rem;
                ...
                border-top:1px solid rgba(0,0,0,0.05);margin-top:1rem;">
    ...
    with _sub_tabs[1]:
        ...
        with _up_col1:
            ...
            <div class="glass-card" style="padding:1.2rem;min-height:240px;">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:0.8rem;">
                    ...
                    <span style="font-size:0.9rem;font-weight:700;color:#0F172A;">Upload Datasets</span>
                </div>
            ...
            <div class="glass-card" style="padding:1.2rem;min-height:240px;">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:0.8rem;">
                    <div style="width:32px;height:32px;background:rgba(139,92,246,0.15);
                        ...
                        display:flex;align-items:center;justify-content:center;font-size:1rem;">📋</div>
                    <span style="font-size:0.9rem;font-weight:700;color:#0F172A;">Loaded Datasets</span>
        ...
        if st.session_state.uploaded_datasets:
            ...
            <div style="background:linear-gradient(90deg,rgba(16,185,129,0.06),rgba(139,92,246,0.04));
                ...
                <span style="color:#64748B;display:block;margin-top:0.3rem;line-height:1.7;">
                        ...
                        for _di, _dcol_name in enumerate(_drow):
                            with _dcols[_di]:
                                ...
                                if len(_dist_data) > 0:
                                    ...
                                    _dfig.update_layout(
                                        **LO, height=200,
                                        title=dict(text=_dcol_name, font=dict(size=11, color="#334155"), x=0.5),
                                        xaxis=dict(showgrid=False, color="#334155", title=""),
                                        yaxis=dict(showgrid=True, gridcolor=GRID, color="#334155", title=""),
                                        showlegend=False
                                    )
                ...
                for _hi_icon, _hi_title, _hi_desc in _health_items:
                    ...
                    <div class="col-drill">
                        <div style="display:flex;align-items:center;gap:10px;">
                            ...
                            <div>
                                <div style="font-size:0.82rem;font-weight:600;color:#0F172A;">{_hi_title}</div>
            ...
            if len(_num_cols_ins) >= 2:
                ...
                st.markdown('<div class="glass-card" style="padding:1.2rem;">', unsafe_allow_html=True)
                st.markdown('<p style="font-size:0.8rem;color:#64748B;margin-bottom:0.8rem;">Pearson correlation between numeric columns. Stronger correlations are highlighted.</p>', unsafe_allow_html=True)
                ...
                try:
                    ...
                    _heatmap_fig = go.Figure(data=go.Heatmap(
                        ...
                        showscale=True,
                        colorbar=dict(tickfont=dict(color="#334155"), title=dict(text="r", font=dict(color="#334155")))
                    ))
                    _heatmap_fig.update_layout(
                        paper_bgcolor="rgba(255,255,255,1)", plot_bgcolor="rgba(255,255,255,1)", font=FONT,
                        height=max(280, len(_num_cols_ins) * 40),
                        margin=dict(t=10, b=10, l=10, r=10),
                        xaxis=dict(color="#334155", tickangle=45),
            ...
            if len(st.session_state.uploaded_datasets) >= 2:
                ...
                st.markdown('<div class="glass-card" style="padding:1.2rem;">', unsafe_allow_html=True)
                st.markdown('<p style="font-size:0.8rem;color:#64748B;margin-bottom:1rem;">Merge two uploaded datasets on a shared key column.</p>', unsafe_allow_html=True)
            ...
            if len(st.session_state.uploaded_datasets) >= 2:
                ...
                st.markdown('<div class="glass-card" style="padding:1.2rem;">', unsafe_allow_html=True)
                st.markdown('<p style="font-size:0.8rem;color:#64748B;margin-bottom:0.8rem;">Compare key statistics side-by-side between two datasets.</p>', unsafe_allow_html=True)
                ...
                if _shared_cols:
                    ...
                    <div style="background:rgba(14,165,233,0.06);border:1px solid rgba(14,165,233,0.2);
                        ...
                        <span style="color:#64748B;font-size:0.78rem;margin-left:0.4rem;">{", ".join(_shared_cols[:10])}{" ..." if len(_shared_cols) > 10 else ""}</span>
                    </div>""", unsafe_allow_html=True)
            ...
            try:
                ...
                if _fig is not None:
                    ...
                    if _chart_title:
                        _fig.update_layout(title=dict(text=_chart_title, font=dict(size=14, color="#0F172A"), x=0.5))
            ...
            <div class="glass-card" style="padding:1.2rem;border-top:3px solid #8B5CF6;">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:0.8rem;">
                    ...
                    <div>
                        <div style="font-size:0.9rem;font-weight:700;color:#0F172A;">AI Dataset Copilot</div>
                    ...
                    try:
                        ...
                        if _df_res is not None and not _df_res.empty:
                            ...
                            <div style="background:linear-gradient(135deg,rgba(139,92,246,0.06),rgba(16,185,129,0.04));
                                ...
                                <p style="color:#0F172A;font-size:0.83rem;margin:0;line-height:1.7;">{_ai_insight_txt}</p>
                            </div>
                            """, unsafe_allow_html=True)
            ...
            st.markdown('<div class="glass-card" style="padding:1.2rem;">', unsafe_allow_html=True)
            st.markdown('<p style="font-size:0.8rem;color:#64748B;margin-bottom:0.8rem;">Generate a comprehensive report of all loaded datasets — download as CSV, Excel, or text.</p>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.saved_queries:
        ...
        for _i,_q in enumerate(st.session_state.saved_queries,1):
            st.markdown(f'<div class="history-item"><div style="font-size:0.88rem;font-weight:600;color:#0F172A!important;">&#128278; {_q}</div></div>',unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for _tc,(_tn2,_ti) in zip(_tcols,_tables.items()):
        with _tc:
            st.markdown(f'<div class="glass-card" style="text-align:center;padding:1rem;"><div style="font-size:1.8rem;">{_ti["emoji"]}</div><div style="font-size:0.85rem;font-weight:700;color:#0F172A!important;margin-top:0.4rem;">{_tn2}</div><div style="font-size:0.72rem;color:#64748B!important;">{_ti["desc"]}</div></div>',unsafe_allow_html=True)
    st.markdown("<br>",unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f'<div style="display:flex;align-items:center;justify-content:space-between;background:rgba(0,0,0,0.02);border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:0.8rem 1rem;margin-bottom:1rem;"><div><div style="font-size:0.85rem;font-weight:600;color:#0F172A!important;">Active Database</div><div style="font-size:0.75rem;color:#64748B!important;">{_dmsg}</div></div><div><span class="{_ddot}"></span><span style="font-size:0.8rem;color:{"#10B981" if _dok else "#EF4444"}!important;">{"Online" if _dok else "Offline"}</span></div></div>',unsafe_allow_html=True)
    
    st.markdown('<p style="font-size:0.82rem;font-weight:700;color:#0F172A;margin-bottom:0.5rem;">Configure Database:</p>', unsafe_allow_html=True)
    ...
    for _ek,_el in [("AZURE_OPENAI_ENDPOINT","OpenAI Endpoint"),("DEPLOYMENT_NAME","Model Deployment"),
        ...
        st.markdown(f'<div style="display:flex;align-items:center;justify-content:space-between;background:rgba(0,0,0,0.02);border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:0.7rem 1rem;margin-bottom:0.4rem;"><div><div style="font-size:0.82rem;font-weight:600;color:#0F172A!important;">{_el}</div><div style="font-size:0.72rem;color:#64748B!important;font-family:monospace;">{_edisp}</div></div><span class="{_edot}"></span></div>',unsafe_allow_html=True)
    st.markdown('<div style="font-size:0.78rem;color:#64748B!important;margin-top:0.5rem;">Edit the <code style="color:#FBB724;">.env</code> file to update credentials.</div></div>',unsafe_allow_html=True)
    ...
    for _ik,_iv in [("Version","1.0.0"),("Framework","Streamlit 1.51"),("Python","3.13"),
        ...
        st.markdown(f'<div style="display:flex;justify-content:space-between;padding:0.4rem 0;border-bottom:1px solid rgba(0,0,0,0.04);"><span style="font-size:0.82rem;color:#64748B!important;">{_ik}</span><span style="font-size:0.82rem;color:#0F172A!important;font-weight:600;">{_iv}</span></div>',unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True)
